<div>
    ...
    <div class="bg-white pb-4 px-4 rounded-md w-full">
        ...
        <div class="w-full flex justify-between px-2 mt-2">
            <a href="/admin/spots/create" wire:navigate
            ...
            <table class="table-auto border-collapse w-full">
                <thead>
                    <tr class="rounded-lg text-sm font-medium text-gray-700 text-left" style="font-size: 0.9674rem">
                        ...
                        <th class="px-4 py-2 " style="background-color:#f8f8f8">Titik Koordinat</th>
                ...
                <tbody class="text-sm font-normal text-gray-700">
                    @forelse ($spots as $item)
                        <tr class="hover:bg-gray-100 border-b border-gray-200 py-10">
                            <td class="px-4 py-4">{{ $item->plant->name }}</td>
                            <td class="px-4 py-4">{{ $item->coordinate }}</td>
                            <td class="px-4 py-4">{{ $item->created_at }}</td>
                            <td class="px-4 py-4">{{ $item->updated_at }}</td>
                            <td class="px-4 py-4">
                                <a href="/admin/spots/{{ $item->id }}/edit" wire:navigate>
                                    <button
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-4">Data lokasi tidak ditemukan!</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
